...
public class Step07ExceptionTest extends PlainTestCase {
    ...
    // done tabata javatryだと、superクラスにlog()があるので、そっち使って大丈夫です by jflute (2026/03/04)
  ...
  /\           |     |        PK           ^^v
    ...
    // これだとhelpThrowIllegalState()で起きた本当の原因を消してしまっている。
        // #1on1: 問題なし、やばいcatchを世の中から減らそう (2026/04/01)
    }
    ...
    public void test_exception_zone_differenceExceptionError() {
        ...
        // _/_/_/_/_/_/_/_/_/_/
        // #1on1: ErrorとException, エラーと例外の違い (2026/04/01)
        /*
                           +-----------------+
                           |                 |
                  +------------------+       |
   ...
   +--------------------+        +-------------------+
     ...
     OutOfMemoryError                     ｜
         ...
         */
        // そもそも、プログラミングの世界における自然言語としてのエラーと例外の違いは？
        // 想定していたものか？想定していないものか？ by たばたさん
        // 
        // エラー: ダメな状態
        // 例外  : ダメではないが、普通ではない (by たばたさん) → 正常なレアケース
        //
        // エラー: システムエラー(バグレベル)、業務エラー(チケット売り切れ)
        // 例外  : システム例外(バグレベル)、業務例外(チケット売り切れ)
        //
        // エラーって修飾子がない場合、多くの場合システムエラーを表現すること多い。
        //
        // throwする人は、全体像を知らないので、(厳密には)(システム)エラーかどうかは判断できない。
        // (確率的に99%エラーとかエラーじゃないとか言えるかもだけど、100%ではない)
        //
        // catchする人(呼び出した人)が、全体像と照らし合わせてエラーかどうか判断する。
        //
        // なので、throwする人は「例外 (ダメではないが、普通ではない)」というニュアンスでしか、
        // throwすることが基本的にはできない。(というかそれが妥当)
        // その「例外 (ダメではないが、普通ではない)」が実際全体としてエラーかどうか？
        // catchする人が判断する。(その例外はエラーだった、もしくは、それ例外は例外だった)
        //
        // ただ、会話上、人間そんな精密に言葉を使い分ける反射神経持ってないので...
        // エラーと言ったり、例外と言ったり、混ざる。
        //
        // TypeScriptの例、Rubyの例、Goの例
        //
        // 再び、エラーって修飾子がない場合、システムエラーを指す文化もあれば、
        // システムエラー、業務エラーの「抽象概念としてのエラー」を指す文化もある。
        //
        // #1on1: こういう概念を深掘りして理解する習慣を付けて欲しい (2026/04/01)
    }
}
